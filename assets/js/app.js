/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `LINUXPLUS` namespace with
   ...
   - quizEngine.js adds the 500+ question bank, the 30-object PBQ database, and
...
(function () {
  ...
  var S = window.LINUXPLUS || {};
  var PASS = (S.exam && S.exam.passing) || 720;
  var CEIL = (S.exam && S.exam.scaleHigh) || 900; // scale ceiling, single source of truth
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("linuxplus." + k, fb); },
      set: function (k, v) { write("linuxplus." + k, v); },
      del: function (k) { try { localStorage.removeItem("linuxplus." + k); } catch (e) {} }
    };
  })();
  ...
  /* Browser-optimization notice, injected above the interactive launch controls
     in Sections 06 (PBQ simulators) and 07 (labs). Same exact wording as the
     dashboard welcome panel in index.html. */
  function card(opts) {
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the XK0-006 is scored and delivered, and where the credential fits in an IT/cybersecurity career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the XK0-006 &mdash; the high-stakes details every candidate should internalize before sitting the test.",
    ...
    g1.appendChild(card({
      ...
      desc: "Where Linux+ sits on the career ladder: a vendor-neutral system administration credential, and the Linux administrator, DevOps, cloud, and site-reliability roles it unlocks.",
      foot: "Foundational · Linux sysadmin track", cta: "Read",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every Linux+ acronym across all five domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the XK0-006 &mdash; 90 questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: full XK0-006 video courses, the official CompTIA objectives PDF, the Linux man pages, and trusted community guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.LINUXPLUS_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. Linux+" });
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates LINUXPLUS.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.LINUXPLUS_VER ? "?v=" + window.LINUXPLUS_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "XK0-006 Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: XK0-006", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where Linux+ fits in your path", S.careerGuidance || [])); }
